...
class TJTwitter2 
{
   ...
   {
      //your code goes here      
      for(TJ_Status2 s: statuses)
      {
         String[] words = removePunctuation(s.getText()).split(" ");
         for(String w: words)
         {
            terms.add(w);
         }
      ...
      for(int k = array.size()-1; k > 0; k--)		
      {
         ...
         for(int j = 1; j <= k; j++)			
            if(array.get(j).compareTo(array.get(maxPos)) > 0)	
               maxPos = j;
         String temp = array.get(maxPos);				
         array.set(maxPos,array.get(k));
         array.set(k,temp);
      }   
   }
   ...
   public String mostPopularWord()
   {
      ...
      String currWord = terms.get(0);
      for(int i = 0; i < terms.size() ; i++)
      {
         if(terms.get(i).equals(currWord))
         {
            currentOc++;
         }
         else
         {
            currWord = terms.get(i); 
            currentOc = 1;  
         }
         if(currentOc > maxOc)
         {
